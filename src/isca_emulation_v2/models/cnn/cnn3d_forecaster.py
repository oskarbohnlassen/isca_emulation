...
from isca_emulation_v2.models.cnn.cnn_decoder import CNN3DDecoder
from isca_emulation_v2.models.cnn.cnn_down_samplers import Downsample3D
from isca_emulation_v2.models.cnn.cnn_encoder import CNN3DEncoder
from isca_emulation_v2.models.cnn.cnn_layers import CNN3DLayer, CNNResBlock3D
...
class SimpleCNN3D(nn.Module):
    def __init__(
        ...
        self.decoder = CNN3DDecoder(hidden_dim=self.hidden, out_channels=self.out_channels)
...
class UnetCNN3D_Fixed(nn.Module):
    """
    Minimal fixed U-Net-style CNN (3D):
    - input proj -> 5x5x5 conv -> downsample
    - 3x3x3 conv -> downsample
    - 3x3x3 bottleneck
    - upsample -> 3x3x3 conv
    - upsample -> 5x5x5 conv -> output proj
    ...
    def __init__(
        ...
        self.from_c3_to_c2 = CNN3DEncoder(
            in_channels=c3,
            hidden_dim=c2,
            activation=self.activation,
            use_batch_norm=self.use_batch_norm,
        )
        self.from_c2_to_c1 = CNN3DEncoder(
            in_channels=c2,
            hidden_dim=c1,
            activation=self.activation,
            use_batch_norm=self.use_batch_norm,
        )
        self.fuse_c2 = CNN3DEncoder(
            in_channels=2 * c2,
            hidden_dim=c2,
            activation=self.activation,
            use_batch_norm=self.use_batch_norm,
        )
        self.fuse_c1 = CNN3DEncoder(
            in_channels=2 * c1,
            hidden_dim=c1,
            activation=self.activation,
            use_batch_norm=self.use_batch_norm,
        ...
        self.output_proj = CNN3DDecoder(hidden_dim=c1, out_channels=self.out_channels)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        h = self.enc_5x5(h)
        skip1 = h
        size_after_enc1 = h.shape[-3:]
        h = self.down_1(h)
        h = self.enc_3x3(h)
        skip2 = h
        size_after_enc2 = h.shape[-3:]
        h = self.down_2(h)
        h = self.bottleneck(h)

        h = F.interpolate(h, size=size_after_enc2, mode="trilinear", align_corners=False)
        h = self.from_c3_to_c2(h)
        h = torch.cat([h, skip2], dim=1)
        h = self.fuse_c2(h)
        h = self.dec_3x3(h)

        h = F.interpolate(h, size=size_after_enc1, mode="trilinear", align_corners=False)
