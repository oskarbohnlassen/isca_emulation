...
from .detokenizer import CNN3DDetokenizer
from .token_processor import GlobalAttentionTokenProcessor, SwinTokenProcessor
from .tokenizer import CNN3DTokenizer


class Transformer3DGlobalAttentionForecaster(nn.Module):
    """Patch-based global-attention transformer forecaster for [B, C, L, H, W] climate fields."""
    ...
    def __init__(
        ...
        grid_width: int,
        hidden_dim: int = 128,
        patch_size: tuple[int, int, int] | int = (5, 4, 4),
        num_layers: int = 4,
        num_heads: int = 4,
        mlp_ratio: float = 4.0,
        activation: str = "gelu",
        ...
        self.detokenizer = CNN3DDetokenizer(
            hidden_dim=self.hidden_dim,
        ...
        self.num_patch_depths = self.tokenizer.num_patch_depths
        ...
        self.patch_volume = self.tokenizer.patch_volume
...
class Transformer3DSwinForecaster(nn.Module):
    """Patch-based shifted-window transformer forecaster for [B, C, L, H, W] climate fields."""
    ...
    def __init__(
        ...
        hidden_dim: int = 128,
        patch_size: tuple[int, int, int] | int = (5, 4, 4),
        window_size: tuple[int, int, int] | int = (2, 4, 4),
        num_layers: int = 4,
        num_heads: int = 4,
        mlp_ratio: float = 4.0,
        activation: str = "gelu",
        ...
        if isinstance(window_size, int):
            self.window_size = (int(window_size), int(window_size), int(window_size))
        else:
        ...
        if (
            self.num_patch_depths % self.window_size[0] != 0
            or self.num_patch_rows % self.window_size[1] != 0
            or self.num_patch_cols % self.window_size[2] != 0
        ):
            raise ValueError(
                "window_size must divide the 3D token grid. "
                f"Received token_grid=({self.num_patch_depths}, {self.num_patch_rows}, {self.num_patch_cols}) "
