...
from isca_emulation_v2.data.utils import build_cnn2d_coord_channels, dump_yaml_mapping, load_isca_result_data
    ...
    *,
    ...
    def append_split(name: str, piece: torch.Tensor) -> None:
        ...
        if stats_path is None and name == stats_source:
            ...
            reduce_dims = (0, *range(2, piece64.dim()))
...
def xarray_to_tensor(
    ...
    *,
    ...
    lon_dim: str,
    time_dim: str,
    load: bool = True,) -> tuple[torch.Tensor, "xr.DataArray"]:
    """Convert selected variables to [T, C, H, W] by stacking (var, level) into channel."""
    ...
    stacked = xr.concat(blocks, dim=xr.IndexVariable("var", list(vars)))
    stacked = stacked.stack(channel=("var", level_dim)).transpose(
        time_dim,
        "channel",
        lat_dim,
        lon_dim,
...
def inverse_scale_cnn2d(scaled_data: np.ndarray, cfg: DictConfig) -> tuple[np.ndarray, np.ndarray]:
    """Unscale the CNN2D output by applying the inverse of the standardization."""
...
class SimpleDatasetCNN2D(IterableDataset):
    ...
    def __init__(
        ...
        self.mean = mean
        self.std = std
        self.coords = build_cnn2d_coord_channels(lonlat_values=lonlat_values, add_coords=add_coords)
    ...
    def _append_coords(self, x_batch: torch.Tensor) -> torch.Tensor:
        if self.coords is None:
            return x_batch
        bsz, x_ch, height, width = x_batch.shape
        coord_ch = int(self.coords.shape[0])
        out = torch.empty(
            (bsz, x_ch + coord_ch, height, width),
            dtype=x_batch.dtype,
            device=x_batch.device,
        )
        out[:, :x_ch] = x_batch
        coords_expanded = self.coords.unsqueeze(0).expand(bsz, -1, -1, -1)
        ...
        if self.shuffle_in_shard:
            ...
            for i in range(0, num_pairs, self.batch_size):
                ...
                if self.coords is not None:
                    coords = self.coords.unsqueeze(0).expand(x_batch.shape[0], -1, -1, -1)
